...
module.exports = function validatePsmbrcrtInput(data, type) {
  ...
  } else if (!Validator.isEmpty(data.psprduid) && data.psprduid.length > 25) {
    errors.psprduid = "INVALIDVALUELENGTH&25";
  }

  if (Validator.isEmpty(data.psitmqty?.toString() || "")) {
    errors.psitmqty = "FIELDISREQUIRED";
  } else if (
    !Validator.isEmpty(data.psitmqty?.toString() || "") &&
    (isNaN(data.psitmqty) || data.psitmqty < 1 || data.psitmqty > 999)
  ) {
