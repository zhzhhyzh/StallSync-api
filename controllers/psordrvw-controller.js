  ...
  try {
    ...
    if (!_.isEmpty(req.body.psrvwimg)) {
      let img_exist = fs.existsSync(
        genConfig.documentTempPath + req.body.psrvwimg
      );
      if (!img_exist)
        return returnError(req, 400, { psrvwimg: "INVALIDDATAVALUE" }, res);
      image = !image;
    }

    if (!_.isEmpty(req.body.psrvwvid)) {
      let img_exist = fs.existsSync(
        genConfig.documentTempPath + req.body.psrvwvid
      );
      if (!img_exist)
        return returnError(req, 400, { psrvwvid: "INVALIDDATAVALUE" }, res);
      video = !video;
    }
...
exports.update = async (req, res) => {
  //Id getting
  const id = req.body.psorduid ? req.body.psorduid : "";
  if (id == "") return returnError(req, 500, "IDRECORDIDISREQUIRED", res);
  ...
  try {
    ...
    if (
      ...
      new Date(exist.updatedAt).getTime() !==
    ...
    await psordrvw
      ...
      .then(async (data) => {
        if (image) {
          if (
            fs.existsSync(genConfig.reviewImgPath + exist.psrvwimg) &&
            exist.psrvwimg
          ) {
        ...
        if (video) {
          if (
            fs.existsSync(genConfig.reviewVidPath + exist.psrvwvid) &&
            exist.psrvwvid
          ) {
      ...
      if (trnscd) {
        psordrvw
          ...
          .then(async () => {
            try {
              if (
                fs.existsSync(genConfig.reviewImgPath + trnscd.psrvwimg) &&
                trnscd.psrvwimg
              ) {
            ...
            try {
              if (
                fs.existsSync(genConfig.reviewVidPath + trnscd.psrvwvid) &&
                trnscd.psrvwvid
              ) {
...
exports.listMerchantReview = async (req, res) => {
  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      obj.createdAt = await common.formatDateTime(obj.createdAt);
      obj.updatedAt = await common.formatDateTime(obj.updatedAt);
